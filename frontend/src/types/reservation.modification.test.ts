...
describe("ReservationModificationResponse", () => {
    ...
    it("should accept a valid declined response without iso_time", () => {
        const response: ReservationModificationResponse = {
            status: "declined",
        ...
        expect(response.iso_time).toBeUndefined();
        expect(response.message).toBe("Unfortunately 7:30pm doesn't work for us.");
    });

    it("should accept response with only status", () => {
        const response: ReservationModificationResponse = {
            status: "declined",
        };
    ...
    it("should match the schema structure", () => {
        ...
        expect(["accepted", "declined"]).toContain(acceptedResponse.status);
        expect(["accepted", "declined"]).toContain(declinedResponse.status);
    });

    it("should allow iso_time to be optional", () => {
        // Type check: iso_time is optional
        const responseWithoutTime: ReservationModificationResponse = {
            status: "declined",
        };

        expect(responseWithoutTime.iso_time).toBeUndefined();
